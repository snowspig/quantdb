...
from data_fetcher.daily_fetcher import DailyFetcher
...
class DailyDataChecker:
    ...
    def _get_trade_dates(self, start_date: str, end_date: str) -> List[str]:
        ...
        try:
            ...
            # 如果没有trade_cal集合或没有数据，尝试从daily集合获取
            logger.info("未找到交易日历数据，尝试从daily集合获取交易日期")
            collection = db[self.collection_name]
            result = collection.distinct("trade_date", {"trade_date": {"$gte": start_date, "$lte": end_date}})
            trade_dates = sorted(result)
            
            if trade_dates:
                logger.info(f"从daily集合获取到 {len(trade_dates)} 个交易日")
                return trade_dates
        ...
        try:
            ...
            if "stock_basic" in db.list_collection_names():
                ...
                if stock_codes:
                    ...
                    logger.warning("从stock_basic集合未获取到股票数据，尝试从daily集合获取")
            else:
                logger.warning("数据库中不存在stock_basic集合，尝试从daily集合获取股票代码")
            
            # 如果从stock_basic获取失败，从daily集合获取
            collection = db[self.collection_name]
            ...
            logger.info(f"从daily集合获取到 {len(stock_codes)} 只目标板块股票")
            return stock_codes
    ...
    def _fetch_with_command_line(self, ts_codes: List[str], start_date: str, end_date: str, batch_size: int, use_parallel: bool = True, batch_delay: int = 5) -> bool:
        ...
        try:
            ...
            logger.error(f"调用daily_fetcher.py失败: {str(e)}")
            import traceback
            logger.debug(f"详细错误信息: {traceback.format_exc()}")
            return False
...
def main():
    ...
    parser.add_argument("--use-cmd", action="store_true", help="使用命令行方式调用daily_fetcher.py，而不是直接导入")
