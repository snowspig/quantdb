#!/usr/bin/env python
"""
repurchase Basic Fetcher V2 - 获取股票回购并保存到MongoDB

该脚本用于从湘财Tushare获取股票回购，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=10124

使用方法：
    python repurchase_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，抓取最近几日的回购数据(默认recent模式)
    python repurchase_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python repurchase_fetcher.py --full            # 使用完整数据模式，分批获取所有回购数据
    python repurchase_fetcher.py --recent 30       # 指定recent模式抓取最近30天的回购数据
    python repurchase_fetcher.py --start-date 20230101 --end-date 20231231  # 指定日期范围抓取数据
    python repurchase_fetcher.py --exchange SZSE   # 获取深交所的股票数据
    python repurchase_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）

注意：repurchase接口支持按日期（ann_date, start_date, end_date）查询，或使用limit/offset分批获取数据
"""
import sys
import time
import json
...
class repurchaseFetcher(TushareFetcher):
    """
    股票回购获取器V2
    
    该类用于从Tushare获取股票回购并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "repurchase.json",
        db_name: str = None,
        collection_name: str = "repurchase",
        start_date: str = None,
        end_date: str = None,
        ...
        recent_mode: bool = True,  # 默认使用recent模式
        recent_days: int = 7,  # recent模式下抓取的天数，默认7天
        full_mode: bool = False,  # 是否使用完整模式
        ...
        Args:
            ...
            recent_mode: 是否使用recent模式(只抓取最近几日数据)
            recent_days: recent模式下抓取的天数，默认7天
            full_mode: 是否使用完整模式(分批获取所有数据)
        ...
        # 检查参数一致性并设置优先级
        # full_mode比recent_mode优先级高
        if self.full_mode and self.recent_mode:
            logger.warning("同时指定了full_mode和recent_mode，将使用full_mode")
        ...
        # 如果未指定日期且使用recent_mode，计算默认日期范围
        if self.recent_mode and not (self.start_date and self.end_date):
            today = datetime.now()
            self.end_date = today.strftime("%Y%m%d")
            past_date = today - timedelta(days=self.recent_days)
            self.start_date = past_date.strftime("%Y%m%d")
            logger.info(f"Recent模式：设置日期范围为过去{self.recent_days}天: {self.start_date} - {self.end_date}")
        ...
        if self.full_mode:
            logger.info("抓取模式: 完整模式(full)，分批抓取所有回购数据")
        elif self.recent_mode:
            logger.info(f"抓取模式: 最近数据模式(recent)，抓取从 {self.start_date} 到 {self.end_date} 的回购数据")
        else:
            if self.start_date and self.end_date:
                logger.info(f"抓取模式: 日期范围模式，抓取从 {self.start_date} 到 {self.end_date} 的回购数据")
            else:
                logger.info("抓取模式: 默认模式，将分批抓取数据")
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        ...
        limit = kwargs.get('limit', 10000)  # 默认获取3000条数据
        ...
        param_msg = []
        if ts_code:
            param_msg.append(f"股票代码={ts_code}")
        if ann_date:
            param_msg.append(f"公告日期={ann_date}")
        if start_date:
            param_msg.append(f"开始日期={start_date}")
        if end_date:
            param_msg.append(f"结束日期={end_date}")
        if limit != 10000 or offset != 0:
            param_msg.append(f"limit={limit} offset={offset}")
        if wan_idx is not None:
            param_msg.append(f"WAN口={wan_idx}")
            
        if param_msg:
            logger.debug(f"【抓取参数】{', '.join(param_msg)}")
        ...
        # 当使用full_mode时，添加limit和offset参数
        if not (ann_date or start_date or end_date):
            params['limit'] = limit
    ...
    def fetch_repurchase_data(self, ann_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定公告日期的数据
        ...
        if not stock_codes:
            logger.error("未能获取到股票代码列表，无法获取股票回购数据")
            return None
    ...
    def _process_date_with_wan(self, ann_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_repurchase_data(ann_date)
                if df is None or df.empty:
                    return False
    ...
    def run(self) -> bool:
        ...
        支持两种模式:
        1. recent模式(默认): 按日期范围抓取最近一段时间的回购数据
        2. full模式: 分批抓取所有回购数据
        ...
        try:
            ...
            if self.full_mode:
                # 使用全量模式，分批获取所有数据
                logger.info("第二步：使用full模式分批抓取所有回购数据")
                return self._run_full_mode()
            else:
                # 使用按日期范围抓取模式
                if self.start_date and self.end_date:
                    logger.info(f"第二步：按日期范围抓取回购数据 ({self.start_date} - {self.end_date})")
                    return self._run_date_range_mode()
                else:
                    # 没有指定日期范围，使用默认的分批抓取
                    logger.info("第二步：未指定日期范围，使用默认分批抓取模式")
                    return self._run_full_mode()
    ...
    def _run_date_range_mode(self) -> bool:
        """
        按日期范围抓取回购数据
        
        使用start_date和end_date参数获取指定日期范围的回购数据
        
        Returns:
            是否成功
        """
        logger.info(f"按日期范围获取回购数据: {self.start_date} - {self.end_date}")
        
        # 使用日期参数抓取数据
        df = self.fetch_data(start_date=self.start_date, end_date=self.end_date)
        
        # 检查是否获取到数据
        if df is None or df.empty:
            logger.info(f"在日期范围 {self.start_date} - {self.end_date} 内未获取到回购数据")
            return True  # 没有数据不视为错误
        
        # 记录获取到的数据量
        record_count = len(df)
        logger.info(f"获取到 {record_count} 条回购数据")
        
        # 处理数据（过滤掉不需要的股票代码）
        processed_df = self.process_data(df)
        if processed_df is None or processed_df.empty:
            logger.info("过滤后没有符合条件的回购数据")
            return True  # 过滤后没有数据不视为错误
        
        # 记录过滤后的数据量
        filtered_count = len(processed_df)
        logger.info(f"过滤后保留 {filtered_count} 条回购数据 (保留00、30、60、68板块)")
        
        # 保存到MongoDB
        success = self.save_to_mongodb(processed_df)
        if not success:
            logger.error(f"保存回购数据到MongoDB失败")
            return False
        
        logger.success(f"成功保存 {filtered_count} 条回购数据到MongoDB")
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取股票回购并保存到MongoDB')
    ...
    parser.add_argument('--start-date', help='公告开始日期，格式：YYYYMMDD')
    parser.add_argument('--end-date', help='公告结束日期，格式：YYYYMMDD')
    parser.add_argument('--db-name', help='MongoDB数据库名称')
    parser.add_argument('--collection-name', default='repurchase', help='MongoDB集合名称')
    ...
    parser.add_argument('--batch-size', type=int, default=3000, help='每批获取的数据条数，默认3000')
    parser.add_argument('--recent', type=int, nargs='?', const=7, help='使用最近数据模式，可指定天数，默认7天')
    parser.add_argument('--full', action='store_true', help='使用完整数据模式，分批获取所有回购数据')
    ...
    try:
        ...
        # 检查模式设置
        recent_mode = True  # 默认使用recent模式
        recent_days = 7     # 默认7天
        full_mode = False   # 默认不使用full模式
        
        # 优先使用full模式
        if args.full:
            full_mode = True
            recent_mode = False
            logger.info("使用完整数据模式(full)")
        elif args.recent is not None:
            recent_mode = True
            recent_days = args.recent
            logger.info(f"使用最近数据模式(recent)，天数: {recent_days}")
        
        # 如果用户明确指定了start_date和end_date，则关闭recent模式
        if args.start_date and args.end_date:
            if recent_mode:
                logger.info(f"用户指定了日期范围 {args.start_date} - {args.end_date}，关闭recent模式")
                recent_mode = False
        ...
        try:
            ...
            if success:
                logger.success("股票回购获取和保存成功")
                return 0
            else:
                logger.error("股票回购获取或保存失败")
                return 1
        except KeyboardInterrupt:
