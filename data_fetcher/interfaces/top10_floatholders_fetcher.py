#!/usr/bin/env python
"""
top10_floatholders Basic Fetcher V2 - 获取前十大流通股东并保存到MongoDB

该脚本用于从湘财Tushare获取前十大流通股东，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=26

使用方法：
    python top10_floatholders_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python top10_floatholders_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python top10_floatholders_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python top10_floatholders_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python top10_floatholders_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python top10_floatholders_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
    python top10_floatholders_fetcher.py --recent          # 使用recent模式，抓取所有股票最近一季度数据
"""
import sys
import time
import json
...
class top10_floatholdersFetcher(TushareFetcher):
    """
    前十大流通股东获取器V2
    
    该类用于从Tushare获取前十大流通股东并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "top10_floatholders.json",
        db_name: str = None,
        collection_name: str = "top10_floatholders",
        start_date: str = None,
        end_date: str = None,
    ...
    def fetch_top10_floatholders_data(self, ann_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定公告日期的数据
        ...
        if not stock_codes:
            logger.error("未能获取到股票代码列表，无法获取前十大流通股东数据")
            return None
    ...
    def _process_date_with_wan(self, ann_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_top10_floatholders_data(ann_date)
                if df is None or df.empty:
                    return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取前十大流通股东并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='top10_floatholders', help='MongoDB集合名称')
    ...
    try:
        ...
        fetcher = top10_floatholdersFetcher(
        ...
        try:
            ...
            if success:
                logger.success("前十大流通股东获取和保存成功")
                return 0
            else:
                logger.error("前十大流通股东获取或保存失败")
                return 1
        except KeyboardInterrupt:
