#!/usr/bin/env python
"""
ggt_monthly Basic Fetcher V2 - 获取港股通每月成交统计数据并保存到MongoDB

该脚本用于从湘财Tushare获取港股通每月成交统计数据，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=26

使用方法：
    python ggt_monthly_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python ggt_monthly_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python ggt_monthly_fetcher.py --start-month 202001 --end-month 202312  # 指定月份范围
    python ggt_monthly_fetcher.py --month 202301,202302,202303           # 指定多个月份
    python ggt_monthly_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python ggt_monthly_fetcher.py --full            # 使用完整模式，获取所有历史数据（从201910开始）
"""
import sys
import time
import json
...
class ggt_monthlyFetcher(TushareFetcher):
    """
    港股通每月成交统计数据获取器V2
    
    该类用于从Tushare获取港股通每月成交统计数据并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "ggt_monthly.json",
        db_name: str = None,
        collection_name: str = "ggt_monthly",
        ...
        full_mode: bool = False,  # 是否使用完整模式（按市场类型抓取）
        ...
        Args:
            ...
            start_month: 开始月份（格式：YYYYMM，默认为当前月份前12个月）
            end_month: 结束月份（格式：YYYYMM，默认为当前月份）
        ...
        # 保存full_mode状态，在full模式下修改月份范围
        if self.full_mode:
            # 在完整模式下，设置月份范围为全部历史数据，从201910开始（根据接口描述）
            today = datetime.now()
            current_month = today.strftime("%Y%m")
            self.start_month = '201910'  # 测试环境数据从201910开始
            self.end_month = current_month
            logger.info(f"完整模式下设置月份范围为: {self.start_month} - {self.end_month}")
        else:
            # 设置默认月份范围（如果未提供）
            if not start_month or not end_month:
                today = datetime.now()
                if not end_month:
                    self.end_month = today.strftime("%Y%m")
                else:
                    self.end_month = end_month
                    
                if not start_month:
                    # 默认获取最近12个月的数据
                    one_year_ago = datetime(today.year - 1, today.month, 1)
                    self.start_month = one_year_ago.strftime("%Y%m")
                else:
        ...
        # 日志输出
        logger.info(f"交易所: {self.exchange}, 月份范围: {self.start_month} - {self.end_month}")
        logger.info(f"处理模式: {'串行' if self.serial_mode else '并行'}, 可用WAN口数量: {self.available_wan_count}, 并行线程数: {self.max_workers}")
        if self.full_mode:
            logger.info("抓取模式: 完整模式(按市场类型)，将抓取所有可用的历史数据")
        else:
            logger.info(f"抓取模式: 月份模式，月份范围: {self.start_month} - {self.end_month}")
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        """
        从Tushare获取港股通每月成交统计数据
        
        Args:
            **kwargs: 查询参数，包括：
                market_type: 市场类型 1：沪股通 2：港股通（沪） 3：深股通 4：港股通（深）
                month: 月度（格式YYYYMM，支持多个输入，如"202001,202002"）
                start_month: 开始月度（格式YYYYMM）
                end_month: 结束月度（格式YYYYMM）
                wan_idx: 指定WAN口索引，可选
        ...
        # 参数检查：至少需要 month 或 start_month+end_month
        if not (month or (start_month and end_month)):
            logger.error("必须提供 month 或 start_month+end_month")
            return None
    ...
    def process_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        由于ggt_monthly接口返回的是市场数据，不需要按股票代码前缀过滤
        ...
        # 直接返回数据，不需要任何过滤
        logger.debug("ggt_monthly接口数据不需要按股票代码过滤")
        return df
    ...
    def get_market_types(self) -> List[str]:
        ...
        用于full模式下，按市场类型抓取所有数据
        
        Returns:
            市场类型列表 ["1", "2", "3", "4"]
        """
        # ggt_monthly接口支持的市场类型:
        # 1：沪股通 2：港股通（沪） 3：深股通 4：港股通（深）
        return ["1", "2", "3", "4"]
    
    def fetch_market_data(self, market_type: str) -> Optional[pd.DataFrame]:
        """
        获取指定市场类型的数据
        ...
        Returns:
            指定市场类型的数据，如果失败则返回None
        """
        logger.info(f"正在获取市场类型 {market_type} 的数据...")
        
        # 创建参数字典
        params = {'market_type': market_type}
        
        # 在full模式下设置最大月份范围（从201910到当前月份）
        if self.full_mode:
            today = datetime.now().strftime("%Y%m")
            params['start_month'] = '201910'
            params['end_month'] = today
            logger.info(f"完整模式下使用月份范围: 201910 - {today}")
        else:
            # 非full模式使用用户指定的月份范围
    ...
    def _process_wan_markets(self, wan_idx, markets):
        """处理单个WAN口对应的所有市场类型"""
        if not markets:  # 没有市场类型需要处理
            logger.info(f"WAN口 {wan_idx} 没有分配到市场类型，跳过")
            return True, 0
        ...
        try:
            logger.info(f"线程开始处理WAN口 {wan_idx} 的 {len(markets)} 个市场类型")
            
            wan_success = True
            success_count = 0
            
            # 在full模式下设置最大月份范围（从201910到当前月份）
            start_month = '201910' if self.full_mode else self.start_month
            end_month = datetime.now().strftime("%Y%m") if self.full_mode else self.end_month
            ...
            for market_type in markets:
                ...
                try:
                    # 打印正在处理的市场类型
                    logger.info(f"WAN口 {wan_idx} 正在处理市场类型: {market_type}")
                    
                    # 获取单个市场数据，直接传递wan_idx参数，同时传递月份参数
                    df = self.fetch_data(
                    ...
                    if df is None or df.empty:
                        logger.warning(f"市场类型 {market_type} 的数据为空或获取失败")
                        continue
                    
                    # 处理数据
                    processed_df = self.process_data(df)
                    if processed_df is None or processed_df.empty:
                        logger.warning(f"市场类型 {market_type} 的处理后数据为空")
                        continue
                    
                    # 保存到MongoDB
                    success = self.save_to_mongodb(processed_df)
                    if success:
                        logger.success(f"市场类型 {market_type} 的数据已保存到MongoDB")
                        success_count += 1
                    else:
                        logger.error(f"保存市场类型 {market_type} 的数据到MongoDB失败")
                        wan_success = False
                except Exception as e:
                    logger.error(f"WAN口 {wan_idx} 处理市场类型 {market_type} 时发生异常: {str(e)}")
                    wan_success = False
            
            # 如果至少成功处理了一个市场类型，视为部分成功
            if success_count > 0:
                logger.info(f"WAN口 {wan_idx} 成功处理了 {success_count}/{len(markets)} 个市场类型")
                wan_success = True
            
            logger.info(f"线程完成WAN口 {wan_idx} 的所有市场类型处理")
            return wan_success, success_count
        finally:
            # 释放WAN口锁
    ...
    def _process_market_parallel(self, market_types: List[str]) -> bool:
        """
        并行处理多个市场类型数据
        ...
        if self.available_wan_count == 0:
            ...
            for market_type in market_types:
                ...
                try:
                    df = self.fetch_market_data(market_type)
                    ...
                    logger.error(f"处理市场类型 {market_type} 时发生异常: {str(e)}")
                    all_success = False
            return all_success
        
        threads_count = min(self.available_wan_count, len(market_types))
        logger.info(f"并行处理 {len(market_types)} 个市场类型的数据，线程数: {threads_count}，可用WAN口: {self.available_wan_indices}")
        ...
        # 均匀分配市场类型到可用WAN口
        market_groups = {wan_idx: [] for wan_idx in self.available_wan_indices[:threads_count]}
        for i, market_type in enumerate(market_types):
            wan_idx = self.available_wan_indices[i % threads_count]
            market_groups[wan_idx].append(market_type)
        
        # 记录分配情况
        for wan_idx, markets in market_groups.items():
            logger.info(f"WAN口 {wan_idx} 分配到 {len(markets)} 个市场类型")
        
        processed_markets_count = 0
        
        # 创建每个WAN口对应的处理函数
        def process_wan_markets(wan_idx, markets):
            """处理单个WAN口对应的所有市场类型"""
            if not markets:  # 没有市场类型需要处理
                logger.info(f"WAN口 {wan_idx} 没有分配到市场类型，跳过")
                return True, 0
        ...
        try:
            # 提交任务，每个WAN口一个任务
            future_to_wan = {}
            for wan_idx in list(market_groups.keys()):
                markets = market_groups.get(wan_idx, [])
                # 即使没有市场类型，也提交任务以保持线程和WAN口一一对应
                logger.info(f"提交WAN口 {wan_idx} 处理 {len(markets)} 个市场类型的任务")
                future = executor_pool.submit(process_wan_markets, wan_idx, markets)
        ...
        # 只有当没有成功处理任何市场类型时，才返回失败
        if processed_markets_count == 0 and len(market_types) > 0:
            logger.error(f"所有 {len(market_types)} 个市场类型都处理失败")
            return False
        
        # 如果至少处理了一些市场类型，就返回成功
        logger.success(f"成功处理了 {processed_markets_count}/{len(market_types)} 个市场类型")
        return True
    ...
    def fetch_ggt_monthly_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的数据
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_ggt_monthly_data(trade_date)
                if df is None or df.empty:
                    return False
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # 对于港股通每月成交统计数据，只支持按市场类型获取
            logger.info(f"使用完整模式，按市场类型获取数据（从{self.start_month}到{self.end_month}）")
            
            # 第二步：获取所有市场类型
            logger.info("第二步：获取所有市场类型列表")
            market_types = self.get_market_types()
            
            if not market_types:
                logger.error("未能获取到任何市场类型，抓取失败")
                return False
            
            # 第三步：获取数据（串行或并行）
            logger.info(f"第三步：处理 {len(market_types)} 个市场类型的数据，月份范围: {self.start_month} - {self.end_month}")
            
            if self.serial_mode:
                # 串行模式
                logger.info(f"使用串行模式处理 {len(market_types)} 个市场类型的数据")
                all_success = True
                ...
                for market_type in market_types:
                    ...
                    logger.info(f"正在处理市场类型: {market_type}")
                    ...
                    try:
                        ...
                        logger.error(f"处理市场类型 {market_type} 的数据时发生异常: {str(e)}")
                        all_success = False
                
                return all_success
            else:
                # 并行模式
                logger.info(f"使用并行模式处理 {len(market_types)} 个市场类型的数据")
                return self._process_market_parallel(market_types)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取港股通每月成交统计数据并保存到MongoDB')
    ...
    parser.add_argument('--start-month', help='开始月份，格式：YYYYMM')
    parser.add_argument('--end-month', help='结束月份，格式：YYYYMM')
    parser.add_argument('--month', help='指定月份(支持多个，如"202001,202002")，格式：YYYYMM')
    parser.add_argument('--db-name', help='MongoDB数据库名称')
    parser.add_argument('--collection-name', default='ggt_monthly', help='MongoDB集合名称')
    ...
    parser.add_argument('--full', action='store_true', help='使用完整模式，获取所有历史数据（从201910开始）')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("港股通每月成交统计数据获取和保存成功")
                return 0
            else:
                logger.error("港股通每月成交统计数据获取或保存失败")
                return 1
