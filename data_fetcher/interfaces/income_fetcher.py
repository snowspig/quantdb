#!/usr/bin/env python
"""
income Basic Fetcher V2 - 获取利润表并保存到MongoDB

该脚本用于从湘财Tushare获取利润表，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=26

使用方法：
    python income_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python income_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python income_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python income_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python income_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python income_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class incomeFetcher(TushareFetcher):
    """
    利润表获取器V2
    
    该类用于从Tushare获取利润表并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "income.json",
        db_name: str = None,
        collection_name: str = "income",
        start_date: str = None,
        end_date: str = None,
    ...
    def fetch_income_data(self, ann_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定公告日期的数据
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取利润表并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='income', help='MongoDB集合名称')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("利润表获取和保存成功")
                return 0
            else:
                logger.error("利润表获取或保存失败")
                return 1
        except KeyboardInterrupt:
