#!/usr/bin/env python
"""
stk_limit Basic Fetcher V2 - 获取每日涨跌停价格数据并保存到MongoDB

该脚本用于从湘财Tushare获取每日涨跌停价格数据，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=26

使用方法：
    python stk_limit_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python stk_limit_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python stk_limit_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python stk_limit_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python stk_limit_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python stk_limit_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class stk_limitFetcher(TushareFetcher):
    """
    每日涨跌停价格数据获取器V2
    
    该类用于从Tushare获取每日涨跌停价格数据并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "stk_limit.json",
        db_name: str = None,
        collection_name: str = "stk_limit",
        start_date: str = None,
        end_date: str = None,
    ...
    def fetch_stk_limit_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的数据
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_stk_limit_data(trade_date)
                if df is None or df.empty:
                    return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取每日涨跌停价格数据并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='stk_limit', help='MongoDB集合名称')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("每日涨跌停价格数据获取和保存成功")
                return 0
            else:
                logger.error("每日涨跌停价格数据获取或保存失败")
                return 1
