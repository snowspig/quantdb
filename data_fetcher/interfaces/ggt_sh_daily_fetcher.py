#!/usr/bin/env python
"""
ggt_sh_daily Basic Fetcher V2 - 获取港股通(沪)每日成交数据并保存到MongoDB

该脚本用于从湘财Tushare获取港股通(沪)每日成交数据，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=10282

使用方法：
    python ggt_sh_daily_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python ggt_sh_daily_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python ggt_sh_daily_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python ggt_sh_daily_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python ggt_sh_daily_fetcher.py --full --start-date 19900101 --end-date 20240101  # 一次性抓取指定日期范围的所有数据
"""
import sys
import time
import json
...
class ggt_sh_dailyFetcher(TushareFetcher):
    """
    港股通(沪)每日成交数据获取器V2
    
    该类用于从Tushare获取港股通(沪)每日成交数据并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "ggt_sh_daily.json",
        db_name: str = None,
        collection_name: str = "ggt_sh_daily",
        start_date: str = None,
        end_date: str = None,
        ...
        full_mode: bool = False,  # 是否使用完整模式（按日期范围一次性抓取所有历史数据）
        ...
        Args:
            ...
            full_mode: 是否使用完整模式（按日期范围一次性抓取所有历史数据）
        ...
        # 保存full_mode状态，在full模式下修改日期范围（如果未提供）
        if self.full_mode:
            # 在完整模式下，如果未提供日期范围则设置为全部历史数据
            today = datetime.now().strftime("%Y%m%d")
            if not start_date:
                self.start_date = '19900101'  # 设置为1990年1月1日起
            if not end_date:
                self.end_date = today
            logger.info(f"完整模式下使用日期范围: {self.start_date} - {self.end_date}")
        else:
            # 设置默认日期范围（如果未提供）
        ...
        if self.full_mode:
            logger.info("抓取模式: 完整模式，一次性抓取指定日期范围内的所有数据")
        else:
            logger.info(f"抓取模式: 日期模式，按交易日分批获取指定日期范围内的数据")
    ...
    def process_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        由于ggt_sh_daily接口返回的是市场数据，不需要按股票代码前缀过滤
        ...
        # 直接返回数据，不需要任何过滤
        logger.debug("ggt_sh_daily接口数据不需要按股票代码过滤")
        return df
    ...
    def fetch_ggt_sh_daily_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的数据
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_ggt_sh_daily_data(trade_date)
                if df is None or df.empty:
                    return False
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # 根据模式走不同的处理流程
            if self.full_mode:
                # 完整模式：直接使用日期范围一次性抓取所有数据
                logger.info(f"使用完整模式，一次性抓取日期范围内的所有数据: {self.start_date} - {self.end_date}")
                
                try:
                    # 直接使用start_date和end_date参数获取所有数据
                    df = self.fetch_data(start_date=self.start_date, end_date=self.end_date)
                    if df is None or df.empty:
                        logger.warning(f"日期范围 {self.start_date} - {self.end_date} 的数据为空或获取失败")
                        return False
                    
                    # 处理数据
                    processed_df = self.process_data(df)
                    if processed_df is None or processed_df.empty:
                        logger.warning(f"日期范围 {self.start_date} - {self.end_date} 的处理后数据为空")
                        return False
                    
                    # 保存数据到MongoDB
                    success = self.save_to_mongodb(processed_df)
                    if success:
                        logger.success(f"日期范围 {self.start_date} - {self.end_date} 的数据已保存到MongoDB")
                        return True
                    else:
                        logger.error(f"保存日期范围 {self.start_date} - {self.end_date} 的数据到MongoDB失败")
                        return False
                except Exception as e:
                    logger.error(f"处理日期范围 {self.start_date} - {self.end_date} 的数据时发生异常: {str(e)}")
                    import traceback
                    logger.debug(f"详细错误信息: {traceback.format_exc()}")
                    return False
            else:
                # 日期模式：按交易日获取
                logger.info(f"使用日期模式，按交易日获取数据: {self.start_date} - {self.end_date}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取港股通(沪)每日成交数据并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='ggt_sh_daily', help='MongoDB集合名称')
    ...
    parser.add_argument('--full', action='store_true', help='使用完整模式，通过start_date和end_date一次性抓取所有数据')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("港股通(沪)每日成交数据获取和保存成功")
                return 0
            else:
                logger.error("港股通(沪)每日成交数据获取或保存失败")
                return 1
