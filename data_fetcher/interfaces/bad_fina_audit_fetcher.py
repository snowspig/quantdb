#!/usr/bin/env python
"""
fina_audit Basic Fetcher V2 - 获取财务审计意见并保存到MongoDB

该脚本用于从湘财Tushare获取财务审计意见，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=26

使用方法：
    python fina_audit_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python fina_audit_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python fina_audit_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python fina_audit_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python fina_audit_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python fina_audit_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class fina_auditFetcher(TushareFetcher):
    """
    财务审计意见获取器V2
    
    该类用于从Tushare获取财务审计意见并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "fina_audit.json",
        db_name: str = None,
        collection_name: str = "fina_audit",
        start_date: str = None,
        end_date: str = None,
        ...
        if self.available_wan_count == 0:
            ...
            if max_workers > self.available_wan_count:
                ...
                self.max_workers = min(max_workers, 2)  # 降低并发数
        ...
        # 新增频率控制参数
        self.min_request_interval = 1.0  # 增加最小请求间隔
        self.global_qps_limit = 35  # 全局最大每秒请求数
        self.last_request_time = {}  # 记录每个WAN口最后请求时间
        self.global_request_times = []  # 记录全局请求时间戳
        self.error_counts = {}  # 记录每个WAN口的错误次数
    ...
    def _get_wan_socket(self, wan_idx=None, retry_count=0):
        ...
        try:
            ...
            now = time.time()
            # 移除1秒前的请求记录
            self.global_request_times = [t for t in self.global_request_times if t > now - 1]
            if len(self.global_request_times) >= self.global_qps_limit:
                wait_time = 1 - (now - self.global_request_times[0])
                logger.warning(f"全局QPS超过限制，等待 {wait_time:.2f} 秒")
                time.sleep(max(wait_time, 0.1))
            
            # WAN口间隔控制
            if wan_idx in self.last_request_time:
                elapsed = now - self.last_request_time[wan_idx]
                if elapsed < self.min_request_interval:
                    wait = self.min_request_interval - elapsed
                    logger.debug(f"WAN口 {wan_idx} 请求间隔不足，等待 {wait:.2f} 秒")
                    time.sleep(wait)
            
            self.last_request_time[wan_idx] = time.time()
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            with closing(self._get_wan_socket(wan_idx=wan_idx)[0]):
            ...
            # 确保端口被释放
            if 'port' in locals() and wan_idx is not None:
                self.port_allocator.release_port(wan_idx, locals()['port'])
    ...
    def fetch_fina_audit_data(self, ann_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定公告日期的数据
    ...
    def _process_date_with_wan(self, ann_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_fina_audit_data(ann_date)
                if df is None or df.empty:
                    return False
        ...
        try:
            if wan_idx in self.error_counts and self.error_counts[wan_idx] > 5:
                logger.warning(f"WAN口 {wan_idx} 错误次数过多，暂停使用")
                time.sleep(30)  # 错误过多时暂停较长时间
                self.error_counts[wan_idx] = 0
    ...
    def get_stock_codes(self) -> List[str]:
        ...
        try:
            ...
            # 非完整模式添加时间过滤
            if not self.full_mode:
                query["list_date"] = {"$lt": self.end_date}
            
            result = self.mongodb_handler.find_documents("stock_basic", query)
            return [doc["ts_code"] for doc in result if "ts_code" in doc]
        except Exception as e:
    ...
    def _process_stocks_parallel(self, stock_codes: List[str]) -> bool:
        """并行处理股票"""
        # 添加更多随机化，分散请求时间
        ...
        logger.info(f"将{len(stock_codes)}个股票分为{len(batches)}批处理，每批{batch_size}个")
        
        # 每批处理完后暂停，避免过度请求
        for i, batch in enumerate(batches):
            if i > 0:
                logger.info(f"等待10秒后处理第{i+1}/{len(batches)}批...")
                time.sleep(10)
    ...
    def _process_batch_parallel(self, batch_codes: List[str]) -> bool:
        """处理一批股票"""
        # 分配股票到WAN口
        stock_groups = {wan_idx: [] for wan_idx in self.available_wan_indices}
        for i, code in enumerate(batch_codes):
            wan_idx = self.available_wan_indices[i % len(self.available_wan_indices)]
            stock_groups[wan_idx].append(code)

        logger.info(f"开始并行处理批次中的 {len(batch_codes)} 个股票")
        
        # 创建线程池
        global executor_pool
        executor_pool = concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers)
        ...
        try:
            # 提交任务
            futures = []
            for wan_idx, codes in stock_groups.items():
                future = executor_pool.submit(self._process_wan_stocks, wan_idx, codes)
        ...
        logger.info(f"本批次成功处理 {processed_stocks_count}/{len(batch_codes)} 个股票")
        return processed_stocks_count > 0

    def _process_wan_stocks(self, wan_idx, stocks):
        """处理单个WAN口对应的所有股票"""
        if not stocks:
        ...
        logger.info(f"线程使用WAN口 {wan_idx} 处理 {len(stocks)} 个股票")
        ...
        try:
            ...
            for i, ts_code in enumerate(stocks):
                ...
                # 添加处理间隔控制
                if i > 0:
                    interval = self.min_request_interval + random.uniform(-0.1, 0.3)
                ...
                try:
                    ...
                    for attempt in range(3):
                        try:
                            logger.info(f"WAN口 {wan_idx} 处理股票: {ts_code}")
                            
                            # 使用fetch_data而不是fetch_stock_data，直接传递wan_idx
                            df = self.fetch_data(ts_code=ts_code, wan_idx=wan_idx)
                            
                            if df is not None and not df.empty:
                                processed_df = self.process_data(df)
                                if processed_df is not None and not processed_df.empty:
                                    if self.save_to_mongodb(processed_df):
                                        logger.info(f"股票 {ts_code} 的数据已保存到MongoDB")
                                        success_count += 1
                                        # 成功后动态调整请求间隔
                                        self._adjust_rate_limit(wan_idx, True)
                                        break
                            
                            # 未成功处理，等待后重试
                            if attempt < 2:  # 最后一次尝试不需要等待
                                backoff = (2 ** attempt) + random.random()
                                logger.warning(f"处理 {ts_code} 未成功，第{attempt+1}次重试，等待{backoff:.1f}秒")
                                time.sleep(backoff)
                        
                        except Exception as e:
                            if "40203" in str(e) or "Socket" in str(e):  # 添加可能的错误类型
                                # 遇到限频错误或Socket错误
                                backoff = (2 ** attempt) + random.random()
                                logger.warning(f"WAN口 {wan_idx} 处理 {ts_code} 遇到错误: {str(e)}")
                                logger.warning(f"第{attempt+1}次重试，等待{backoff:.1f}秒")
                                
                                # 更新错误计数
                                self.error_counts[wan_idx] = self.error_counts.get(wan_idx, 0) + 1
                                # 动态调整请求间隔(变慢)
                                self._adjust_rate_limit(wan_idx, False)
                    ...
                    logger.error(f"WAN口 {wan_idx} 处理 {ts_code} 失败: {str(e)}")
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if not stock_codes:
                logger.error("未找到有效股票代码")
                return False

            logger.info(f"开始处理 {len(stock_codes)} 支股票，时间范围: {self.start_date}-{self.end_date}")

            # 根据模式选择处理方式
            if self.serial_mode:
                return self._process_stocks_serial(stock_codes)
            return self._process_stocks_parallel(stock_codes)
            
        except Exception as e:  # 添加缺失的except子句
    ...
    # 根据错误情况动态调整请求间隔
    def _adjust_rate_limit(self, wan_idx, success=True):
        if success:
            # 成功则稍微降低间隔
            self.min_request_interval = max(0.5, self.min_request_interval * 0.9)
        else:
            # 失败则增加间隔
            self.min_request_interval = min(5.0, self.min_request_interval * 1.5)
            logger.info(f"调整请求间隔为 {self.min_request_interval:.1f} 秒")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取财务审计意见并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='fina_audit', help='MongoDB集合名称')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("财务审计意见获取和保存成功")
                return 0
            else:
                logger.error("财务审计意见获取或保存失败")
                return 1
