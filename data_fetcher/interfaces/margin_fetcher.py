#!/usr/bin/env python
"""
margin Basic Fetcher V2 - 获取融资融券交易汇总并保存到MongoDB

该脚本用于从湘财Tushare获取融资融券交易汇总，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=10218

使用方法：
    python margin_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python margin_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python margin_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python margin_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python margin_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python margin_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class marginFetcher(TushareFetcher):
    """
    融资融券交易汇总获取器V2
    
    该类用于从Tushare获取融资融券交易汇总并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "margin.json",
        db_name: str = None,
        collection_name: str = "margin",
        start_date: str = None,
        end_date: str = None,
        ...
        if not start_date or not end_date:
            ...
            if not start_date:
                if self.full_mode:
                    # full模式下，默认使用1990年1月1日作为开始日期
                    self.start_date = "19900101"
                    logger.info(f"full模式：使用历史全量数据，开始日期设为: {self.start_date}")
                else:
                    # 默认获取最近一周的数据
        ...
        if self.full_mode:
            logger.info("抓取模式: 完整模式(按交易所)，将抓取所有历史数据")
        else:
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        ...
        Args:
            **kwargs: 查询参数，包括：
                ...
                start_date: 开始日期
                end_date: 结束日期
                exchange_id: 交易所代码（SSE：上交所，SZSE：深交所）
                wan_idx: 指定WAN口索引，可选
        ...
        exchange_id = kwargs.get('exchange_id')  # 增加对exchange_id的处理
        ...
        if exchange_id:
            params['exchange_id'] = exchange_id  # 确保exchange_id正确传递
        
        # 参数检查：现在至少需要 trade_date 或 start_date+end_date 或 ts_code+exchange_id
        if not (trade_date or (start_date and end_date and (ts_code or exchange_id))):
            logger.error("必须提供 ts_code、trade_date 或 start_date+end_date 以及 exchange_id")
            return None
    ...
    def fetch_margin_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的数据
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_margin_data(trade_date)
                if df is None or df.empty:
                    return False
    ...
    def fetch_stock_data(self, ts_code: str = None, exchange_id: str = None) -> Optional[pd.DataFrame]:
        """获取指定股票代码或交易所的数据"""
        if ts_code:
            logger.info(f"正在获取股票 {ts_code} 的基本数据...")
        elif exchange_id:
            logger.info(f"正在获取交易所 {exchange_id} 的融资融券数据...")
        else:
            logger.error("必须提供 ts_code 或 exchange_id 参数")
            return None
        
        # 创建参数字典
        params = {}
        
        # 设置ts_code或exchange_id参数
        if ts_code:
            params['ts_code'] = ts_code
        ...
        # 如果不是full模式且有ts_code，添加exchange参数
        if not self.full_mode and ts_code:
            params['exchange'] = self.exchange
        
        # 始终添加日期范围
        logger.info(f"使用日期范围: {self.start_date} 到 {self.end_date}")
    ...
    def _process_stock_parallel(self, stock_codes: List[str] = None) -> bool:
        """
        并行处理多个股票代码数据或交易所数据
        
        Args:
            stock_codes: 股票代码列表，在full模式下忽略
            
        Returns:
            是否全部成功
        """
        # 在full模式下使用交易所列表而不是股票代码列表
        exchanges = ["SSE", "SZSE"] if self.full_mode else None
        ...
        if self.available_wan_count == 0:
            ...
            # 根据模式选择处理的项目
            if self.full_mode:
                items_to_process = exchanges
                item_type = "交易所"
            else:
                items_to_process = stock_codes
                item_type = "股票"
            ...
            for item in items_to_process:
                ...
                try:
                    if self.full_mode:
                        logger.info(f"正在处理交易所: {item}")
                        df = self.fetch_stock_data(exchange_id=item)
                    else:
                        logger.info(f"正在处理股票: {item}")
                        df = self.fetch_stock_data(ts_code=item)
                    
                    if df is not None and not df.empty:
                        logger.info(f"获取到 {item_type} {item} 的数据，共 {len(df)} 条记录")
                        processed_df = self.process_data(df)
                        if processed_df is not None and not processed_df.empty:
                            logger.info(f"处理后的 {item_type} {item} 数据，共 {len(processed_df)} 条记录")
                            success = self.save_to_mongodb(processed_df)
                            if success:
                                logger.success(f"成功保存 {item_type} {item} 的数据到MongoDB")
                            else:
                                logger.error(f"保存{item_type} {item} 的数据到MongoDB失败")
                        else:
                            logger.warning(f"{item_type} {item} 的处理后数据为空，可能不存在符合条件的记录")
                    else:
                        logger.warning(f"{item_type} {item} 未获取到数据，可能该交易所在此日期范围内没有融资融券数据")
                    ...
                    logger.error(f"处理{item_type} {item} 时发生异常: {str(e)}")
                    import traceback
        ...
        threads_count = min(self.available_wan_count, self.max_workers)
        
        # 根据模式确定要处理的项目和日志信息
        if self.full_mode:
            items_to_process = exchanges
            item_type = "交易所"
            logger.info(f"并行处理 {len(items_to_process)} 个交易所的数据，线程数: {threads_count}，可用WAN口: {self.available_wan_indices}")
        else:
            items_to_process = stock_codes
            item_type = "股票"
            logger.info(f"并行处理 {len(items_to_process)} 个股票的数据，线程数: {threads_count}，可用WAN口: {self.available_wan_indices}")
        ...
        # 均匀分配要处理的项目到可用WAN口
        item_groups = {wan_idx: [] for wan_idx in self.available_wan_indices}
        for i, item in enumerate(items_to_process):
            wan_idx = self.available_wan_indices[i % len(self.available_wan_indices)]
            item_groups[wan_idx].append(item)
        
        # 记录分配情况
        for wan_idx, items in item_groups.items():
            logger.info(f"WAN口 {wan_idx} 分配到 {len(items)} 个{item_type}")
        
        processed_items_count = 0
        
        # 创建每个WAN口对应的处理函数
        def process_wan_items(wan_idx, items):
            """处理单个WAN口对应的所有项目"""
            if not items:  # 没有项目需要处理
                logger.info(f"WAN口 {wan_idx} 没有分配到{item_type}，跳过")
                return True, 0
            ...
            try:
                logger.info(f"线程开始处理WAN口 {wan_idx} 的 {len(items)} 个{item_type}")
                ...
                for item in items:
                    ...
                    try:
                        # 打印正在处理的项目
                        logger.info(f"WAN口 {wan_idx} 正在处理{item_type}: {item}")
                        
                        # 根据模式获取数据
                        if self.full_mode:
                            # 在full模式下，使用fetch_stock_data而不是直接调用fetch_data
                            df = self.fetch_stock_data(exchange_id=item)
                        else:
                            df = self.fetch_stock_data(ts_code=item)
                        
                        if df is None or df.empty:
                            logger.warning(f"{item_type} {item} 的数据为空或获取失败，可能该交易所在此日期范围内没有融资融券数据")
                            continue
                        
                        logger.info(f"WAN口 {wan_idx} 获取到 {item_type} {item} 的数据，共 {len(df)} 条记录")
                        ...
                        logger.info(f"WAN口 {wan_idx} 处理后的 {item_type} {item} 数据，共 {len(processed_df)} 条记录")
                        
                        # 保存到MongoDB
                        success = self.save_to_mongodb(processed_df)
                        if success:
                            logger.success(f"WAN口 {wan_idx} 成功保存 {item_type} {item} 的数据到MongoDB")
                            success_count += 1
                        else:
                            logger.error(f"WAN口 {wan_idx} 保存{item_type} {item} 的数据到MongoDB失败")
                            wan_success = False
                        
                        # 在full模式下，添加2-5秒的随机延时
                        if self.full_mode:
                            delay = random.uniform(2, 5)
                            logger.info(f"WAN口 {wan_idx} full模式：添加 {delay:.2f} 秒延时，避免频繁请求")
                            time.sleep(delay)
                        
                    except Exception as e:
                        logger.error(f"WAN口 {wan_idx} 处理{item_type} {item} 时发生异常: {str(e)}")
                        import traceback
                        logger.debug(f"异常详细信息: {traceback.format_exc()}")
                        wan_success = False
                
                # 如果至少成功处理了一个项目，视为部分成功
                if success_count > 0:
                    logger.info(f"WAN口 {wan_idx} 成功处理了 {success_count}/{len(items)} 个{item_type}")
                    wan_success = True
                else:
                    logger.warning(f"WAN口 {wan_idx} 没有成功处理任何{item_type}，这可能正常，因为在指定的时间范围内可能没有融资融券数据")
                    # 对于融资融券数据，将无数据视为正常情况，返回成功
                    wan_success = True
                
                logger.info(f"线程完成WAN口 {wan_idx} 的所有{item_type}处理")
                return wan_success, success_count
            finally:
                # 释放WAN口锁
                self.wan_locks[wan_idx].release()
        ...
        try:
            ...
            future_to_wan = {}
            for wan_idx in self.available_wan_indices:
                items = item_groups.get(wan_idx, [])
                # 即使没有项目，也提交任务以保持线程和WAN口一一对应
                logger.info(f"提交WAN口 {wan_idx} 处理 {len(items)} 个{item_type}的任务")
                future = executor_pool.submit(process_wan_items, wan_idx, items)
                future_to_wan[future] = wan_idx
        ...
        # 只有当没有成功处理任何项目时，才返回失败
        if processed_items_count == 0 and len(items_to_process) > 0:
            logger.error(f"所有 {len(items_to_process)} 个{item_type}都处理失败")
            return False
        
        # 如果至少处理了一些项目，就返回成功
        logger.success(f"成功处理了 {processed_items_count}/{len(items_to_process)} 个{item_type}")
        return True
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if self.full_mode:
                # 完整模式：使用交易所代码获取
                logger.info("使用完整模式，按交易所代码获取所有历史数据")
                logger.info(f"数据时间范围: {self.start_date} - {self.end_date}")
                
                # 定义交易所列表
                exchanges = ["SSE", "SZSE"]
                logger.info(f"将使用交易所列表: {exchanges}")
                
                # 获取数据（串行或并行）
                logger.info(f"第二步：处理 {len(exchanges)} 个交易所的数据")
                
                if self.serial_mode:
                    # 串行模式
                    logger.info(f"使用串行模式处理 {len(exchanges)} 个交易所的数据")
                    all_success = True
                    ...
                    for exchange_id in exchanges:
                        ...
                        logger.info(f"正在处理交易所: {exchange_id}")
                        
                        try:
                            # 获取单个交易所数据
                            df = self.fetch_stock_data(exchange_id=exchange_id)
                            if df is None or df.empty:
                                logger.warning(f"交易所 {exchange_id} 的数据为空或获取失败")
                                continue
                            
                            # 处理数据
                            processed_df = self.process_data(df)
                            if processed_df is None or processed_df.empty:
                                logger.warning(f"交易所 {exchange_id} 的处理后数据为空")
                                continue
                            
                            # 保存数据到MongoDB
                            success = self.save_to_mongodb(processed_df)
                            if success:
                                logger.success(f"交易所 {exchange_id} 的数据已保存到MongoDB")
                            else:
                                logger.error(f"保存交易所 {exchange_id} 的数据到MongoDB失败")
                                all_success = False
                            ...
                            logger.error(f"处理交易所 {exchange_id} 的数据时发生异常: {str(e)}")
                            all_success = False
                    
                    return all_success
                else:
                    # 并行模式
                    logger.info(f"使用并行模式处理 {len(exchanges)} 个交易所的数据")
                    return self._process_stock_parallel()
            else:
                # 日期模式：按交易日获取
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取融资融券交易汇总并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='margin', help='MongoDB集合名称')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("融资融券交易汇总获取和保存成功")
                return 0
            else:
                logger.error("融资融券交易汇总获取或保存失败")
                return 1
