...
使用方法：
    python daily_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python daily_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python daily_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python daily_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python daily_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python daily_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class DailyBasicFetcher(TushareFetcher):
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # 准备API参数
            api_name = self.interface_config.get("api_name", "daily")
