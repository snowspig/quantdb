#!/usr/bin/env python
"""
Div Fetcher V2 - 获取日线基本数据并保存到MongoDB
...
使用方法：
    python div_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python div_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python div_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python div_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python div_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python div_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class DivFetcher(TushareFetcher):
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        ...
        Args:
            **kwargs: 查询参数，包括：
                ts_code: 股票代码
                trade_date: 交易日期（内部会映射为ex_date使用）
                ex_date: 除权除息日期
                start_date: 开始日期
                end_date: 结束日期
                wan_idx: 指定WAN口索引，可选
        ...
        # 重要：如果提供了trade_date但没有提供ex_date，则将trade_date的值用作ex_date
        if trade_date and not ex_date:
            params['ex_date'] = trade_date
            logger.debug(f"将trade_date参数值 {trade_date} 映射为ex_date参数")
        elif ex_date:
    ...
    def fetch_div_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的除权除息数据（内部将trade_date映射为ex_date使用）
        
        Args:
            trade_date: 交易日期，格式为YYYYMMDD（内部将作为ex_date使用）
            
        Returns:
            指定日期的除权除息数据，如果失败则返回None
        """
        logger.info(f"正在获取交易日 {trade_date} 的除权除息数据...")
        # 将trade_date作为ex_date参数传递
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_div_data(trade_date)
                if df is None or df.empty:
                    return False
    ...
    def _process_date_with_wan_no_lock(self, trade_date: str, wan_idx: int) -> bool:
        ...
        try:
            ...
            if df is None or df.empty:
                error_msg = f"交易日 {trade_date} 的除权除息数据为空或获取失败"
                logger.warning(error_msg)
                self.error_messages.append(error_msg)  # 添加到错误消息列表
                return False
    ...
    def run(self) -> bool:
        ...
        try:
            # 设置索引字段
            index_fields = ["ts_code", "div_proc", "ann_date", "ex_date"]
            ...
            result = self._process_date_parallel(trade_dates)
...
def main():
    ...
    parser.add_argument('--collection-name', default='div', help='MongoDB集合名称')
    ...
    try:
        ...
        fetcher = DivFetcher(
            ...
            mongo_handler_instance=mongodb_handler  # 传递已初始化的MongoDB实例
        )
        
        # 命令行参数验证
        if args.start_date and args.end_date:
            if args.start_date > args.end_date:
                logger.error(f"开始日期 {args.start_date} 不能晚于结束日期 {args.end_date}")
                return 1
        ...
        # 如果成功为False，且错误信息中包含"为空或获取失败"，则返回2表示无数据
        if not success and any("为空或获取失败" in msg for msg in fetcher.error_messages):
            # 检查是否所有错误都是无数据错误
            all_no_data_errors = all("为空或获取失败" in msg for msg in fetcher.error_messages)
            if all_no_data_errors:
                return 2  # 表示正常执行但没有数据
            else:
                return 1  # 其他错误
        ...
        logger.exception(f"程序执行过程中出现异常: {str(e)}")
        return 1
        
    finally:
        # 确保MongoDB连接被正确关闭
        # MongoDB Handler可能没有close方法，检查一下
        if 'mongodb_handler' in locals() and mongodb_handler:
            # 使用hasattr检查对象是否有close方法
            if hasattr(mongodb_handler, 'close'):
