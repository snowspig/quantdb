#!/usr/bin/env python
"""
Moneyflow Basic Fetcher V2 - 获取日线基本数据并保存到MongoDB
...
使用方法：
    python moneyflow_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python moneyflow_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python moneyflow_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python moneyflow_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python moneyflow_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python moneyflow_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class moneyflowFetcher(TushareFetcher):
    ...
    def fetch_moneyflow_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的数据
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_moneyflow_data(trade_date)
                if df is None or df.empty:
                    return False
...
def main():
    ...
    parser.add_argument('--collection-name', default='moneyflow', help='MongoDB集合名称')
