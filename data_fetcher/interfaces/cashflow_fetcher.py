#!/usr/bin/env python
"""
cashflow Basic Fetcher V2 - 获取现金流量表并保存到MongoDB

该脚本用于从湘财Tushare获取现金流量表，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=26

使用方法：
    python cashflow_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python cashflow_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python cashflow_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python cashflow_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python cashflow_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python cashflow_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class cashflowFetcher(TushareFetcher):
    """
    现金流量表获取器V2
    
    该类用于从Tushare获取现金流量表并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "cashflow.json",
        db_name: str = None,
        collection_name: str = "cashflow",
        start_date: str = None,
        end_date: str = None,
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # 使用通用的get_data方法代替直接调用cashflow方法
            df = client.get_data(api_name='cashflow', params=params)
    ...
    def fetch_cashflow_data(self, ann_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定公告日期的数据
    ...
    def _process_date_with_wan(self, ann_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_cashflow_data(ann_date)
                if df is None or df.empty:
                    return False
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # 根据模式走不同的处理流程
            if self.full_mode:
                # 完整模式：使用limit+offset模式批量获取所有现金流量表数据
                logger.info("使用完整模式，批量获取所有现金流量表数据")
                ...
                # 日期模式：按天获取数据
                logger.info("使用日期模式，按公告日期获取现金流量表数据")
                ...
                if self.serial_mode or len(trade_dates) <= 3:  # 如果日期很少，强制使用串行模式
                    ...
                    for trade_date in trade_dates:
                        ...
                        try:
                            df = self.fetch_cashflow_data(trade_date)
                            if df is None:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取现金流量表并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='cashflow', help='MongoDB集合名称')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("现金流量表获取和保存成功")
                return 0
            else:
                logger.error("现金流量表获取或保存失败")
                return 1
        except KeyboardInterrupt:
