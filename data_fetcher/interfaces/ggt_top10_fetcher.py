#!/usr/bin/env python
"""
ggt_top10 Basic Fetcher V2 - 获取港股通十大成交股并保存到MongoDB

该脚本用于从湘财Tushare获取港股通十大成交股，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=26

使用方法：
    python ggt_top10_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python ggt_top10_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python ggt_top10_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python ggt_top10_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python ggt_top10_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python ggt_top10_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class ggt_top10Fetcher(TushareFetcher):
    """
    港股通十大成交股获取器V2
    
    该类用于从Tushare获取港股通十大成交股并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "ggt_top10.json",
        db_name: str = None,
        collection_name: str = "ggt_top10",
        start_date: str = None,
        end_date: str = None,
        ...
        full_mode: bool = False,  # 是否使用完整模式（按日期抓取）
        ...
        if self.full_mode:
            logger.info("抓取模式: 完整模式(按日期)，将抓取从1990年至今的所有历史数据")
        else:
    ...
    def fetch_ggt_top10_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的数据
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_ggt_top10_data(trade_date)
                if df is None or df.empty:
                    return False
    ...
    def _process_date_with_wan_no_lock(self, trade_date: str, wan_idx: int) -> bool:
        ...
        try:
            ...
            # 记录获取到的数据信息
            logger.info(f"获取到交易日 {trade_date} 的数据，共 {len(df)} 条记录")
            ...
            logger.info(f"处理后的数据，共 {len(processed_df)} 条记录")
            ...
            if success:
                ...
                logger.info(f"WAN口 {wan_idx}：添加 {delay:.2f} 秒延时，避免频繁请求")
                time.sleep(delay)
            else:
    ...
    def fetch_by_date_range(self, start_date: str, end_date: str, market_type: str = None) -> Optional[pd.DataFrame]:
        """
        按日期范围获取港股通十大成交股数据
        ...
        Args:
            start_date: 开始日期，格式为YYYYMMDD
            end_date: 结束日期，格式为YYYYMMDD
            market_type: 市场类型，2为沪市，4为深市，None为全部
            
        Returns:
            日期范围内的数据，如果失败则返回None
        """
        logger.info(f"按日期范围获取数据: {start_date} 至 {end_date}" + (f", 市场: {market_type}" if market_type else ""))
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # 根据模式走不同的处理流程
            if self.full_mode:
                # 完整模式：直接按整体日期范围获取所有历史数据
                logger.info("使用完整模式，直接按整体日期范围获取所有历史数据")
                logger.info(f"日期范围: {self.start_date} - {self.end_date}")
                
                all_success = True
                
                # 直接处理整个时间范围，不再分年
                logger.info("直接获取整个时间范围的数据")
                
                # 分别处理沪市和深市
                for market in ["2", "4"]:  # 2为沪市，4为深市
                    logger.info(f"处理市场 {market} ({'沪市' if market == '2' else '深市'}) 的数据")
                    
                    try:
                        # 获取该市场的数据
                        df = self.fetch_by_date_range(self.start_date, self.end_date, market)
                        
                        if df is None or df.empty:
                            logger.warning(f"市场 {market} 在 {self.start_date}-{self.end_date} 期间无数据")
                            continue
                            
                        # 记录获取到的数据信息
                        logger.info(f"获取到 {self.start_date}-{self.end_date} 市场 {market} 的数据，共 {len(df)} 条记录")
                        
                        # 处理数据
                        processed_df = self.process_data(df)
                        if processed_df is None or processed_df.empty:
                            logger.warning(f"市场 {market} 在 {self.start_date}-{self.end_date} 期间的处理后数据为空")
                            continue
                        ...
                        success = self.save_to_mongodb(processed_df)
                        if success:
                            logger.success(f"成功保存 {self.start_date}-{self.end_date} 市场 {market} 的数据")
                        else:
                            logger.error(f"保存 {self.start_date}-{self.end_date} 市场 {market} 的数据失败")
                            all_success = False
                            
                        # 添加延时避免API限流
                        delay = random.uniform(5, 10)
                        logger.info(f"添加 {delay:.2f} 秒延时，避免API限流")
                        time.sleep(delay)
                        
                    except Exception as e:
                        logger.error(f"处理 {self.start_date}-{self.end_date} 市场 {market} 数据时发生异常: {str(e)}")
                        import traceback
    ...
    def save_to_mongodb(self, df: pd.DataFrame, max_retries=3, chunk_size=10000) -> bool:
        """
        保存数据到MongoDB，优化版本：解决更新中_id字段冲突问题
        ...
        try:
            ...
            for i in range(0, total_records, chunk_size):
                ...
                while retries < max_retries and not success:
                    try:
                        # 1. 提取唯一标识符字段名
                        key_fields = self.index_fields.copy()
                        ...
                        for doc in chunk:
                            ...
                            # 构建更新文档，但移除_id字段以避免冲突
                            update_doc = doc.copy()
                            if '_id' in update_doc:
                                del update_doc['_id']
                            
                            # 添加upsert操作 - 如果存在则更新，不存在则插入
                        ...
                        if bulk_operations:
                            ...
                            skipped_count += (matched_count - result.modified_count)
                            
                            logger.debug(
                                f"批次结果: 匹配: {matched_count}, 插入: {result.upserted_count}, "
                                f"更新: {result.modified_count}, 跳过: {matched_count - result.modified_count}"
                            )
    ...
    def _ensure_collection_and_indexes(self) -> bool:
        ...
        try:
            ...
            if hasattr(self, 'interface_config') and 'index_fields' in self.interface_config:
                ...
                # 如果无法从配置读取，则使用默认索引字段（针对 ggt_top10 的专用索引）
                self.index_fields = ['ts_code', 'trade_date', 'market_type']
                logger.info(f"无法从接口配置读取索引字段，使用默认值: {self.index_fields}")
            ...
            # 创建复合唯一索引
            logger.info(f"正在为集合 {self.collection_name} 创建复合唯一索引 {self.index_fields}...")
            composite_index_key = [(field, pymongo.ASCENDING) for field in self.index_fields]
            
            # 检查是否已存在同样的索引
            composite_index_exists = False
            for index_name, index_info in existing_indexes.items():
                if index_name != "_id_":
                    index_key = [(k, d) for k, d in index_info['key']]
                    if sorted(index_key) == sorted(composite_index_key) and index_info.get('unique', False):
                        composite_index_exists = True
                        logger.info(f"复合唯一索引 {index_name} 已存在")
                        break
            ...
            if not composite_index_exists:
                ...
                logger.info(f"已为字段 {self.index_fields} 创建唯一复合索引")
            
            # 为每个字段创建单字段索引
            for field in self.index_fields:
                index_key = [(field, pymongo.ASCENDING)]
                
                # 检查是否已存在
                field_index_exists = False
                for index_name, index_info in existing_indexes.items():
                    if index_name != "_id_" and index_info['key'] == index_key:
                        field_index_exists = True
                        logger.info(f"字段 {field} 的索引已存在")
                        break
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取港股通十大成交股并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='ggt_top10', help='MongoDB集合名称')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("港股通十大成交股获取和保存成功")
                return 0
            else:
                logger.error("港股通十大成交股获取或保存失败")
                return 1
