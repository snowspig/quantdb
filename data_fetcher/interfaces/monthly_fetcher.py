#!/usr/bin/env python
"""
月线行情数据获取器 - 获取月线行情数据并保存到MongoDB

该脚本用于从湘财Tushare获取月线行情数据，并保存到MongoDB数据库中
该版本通过分时间段获取和多WAN接口并行处理，解决大量数据获取问题

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=10144

使用方法：
    python monthly_fetcher.py              # 默认使用recent模式获取最近三个月的月末交易日数据
    python monthly_fetcher.py --full        # 获取完整历史数据而非默认的最近三个月数据
    python monthly_fetcher.py --verbose     # 使用详细日志模式
    python monthly_fetcher.py --mock        # 使用模拟数据模式（API不可用时）
    python monthly_fetcher.py --start-date 20100101 --end-date 20201231  # 指定日期范围获取数据
    python monthly_fetcher.py --recent      # 显式指定recent模式（最近三个月的月末交易日数据，默认模式）
"""
import os
import sys
import json
...
class MonthlyFetcher:
    ...
    该类用于从Tushare获取月线行情数据并保存到MongoDB数据库
    优化点：
    ...
    def filter_monthly_data(self, df: pd.DataFrame, target_ts_codes: Set[str]) -> pd.DataFrame:
        """
        根据目标股票代码集合过滤月线数据
        ...
        if df is None or df.empty:
            logger.warning("没有月线数据可过滤")
            return pd.DataFrame()
        
        logger.info(f"过滤前月线数据数量: {len(df)}")
        ...
        # 输出过滤统计信息
        logger.info(f"过滤后月线数据数量: {len(df_filtered)}")
        ...
        if self.verbose:
            ...
            if not df_filtered.empty and 'ts_code' in df_filtered.columns:
                # 从ts_code提取市场代码
                # 不再添加market_code字段
                market_stats = df_filtered['ts_code'].str[:2].value_counts().to_dict()
    ...
    def _get_recent_months(self, months: int = 3) -> Tuple[str, str]:
        """
        获取最近几个月的日期范围
        """
        end_date = datetime.now()
        # Calculate months back instead of weeks
        start_date = datetime(end_date.year - (months // 12), 
                             end_date.month - (months % 12) if end_date.month > (months % 12) else end_date.month + 12 - (months % 12), 
                             1)  # Start from beginning of the month for monthly data
        return (
    ...
    async def _fetch_monthly_by_date(self, date: str) -> pd.DataFrame:
        """
        根据交易日期获取月线数据
        """
        # 实际场景中可能需要使用实际绑定IP地址
    ...
    async def _fetch_monthly_by_ts_code(self, ts_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """
        根据股票代码获取月线数据
        """
        # 实际场景中可能需要使用实际绑定IP地址
    ...
    async def _fetch_monthly_by_date_range_for_stock(self, ts_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """
        获取指定股票指定日期范围的月线数据
        """
        return await self._fetch_monthly_by_ts_code(ts_code, start_date, end_date)
        ...
        for ts_code in stock_batch:
            try:
                ...
                if not df.empty:
                    ...
                    logger.info(f"获取股票 {ts_code} 的月线数据: {len(records)} 条记录")
                else:
                    logger.warning(f"股票 {ts_code} 的月线数据为空")
    ...
    def _mock_monthly_data(self) -> List[Dict]:
        """
        生成模拟月线数据（仅用于测试）
        """
        mock_data = []
    ...
    async def fetch_recent_data(self):
        ...
        获取最近几个月的数据更新
        """
        start_date, end_date = self._get_recent_months(months=3)
        logger.info(f"获取最近几个月数据: 从 {start_date} 到 {end_date}")
        ...
        # 按月进行采集，避免单次请求数据过多
        for date_batch in self._split_list(trade_dates, 20):  # 月数据量一般较周数据少，可以增大批次
            # 每月最后一个交易日当作月数据点
            monthly_date = date_batch[-1]
            logger.info(f"处理月数据日期: {monthly_date}")
            
            if self.mock:
                data = self._mock_monthly_data()
                logger.info(f"模拟月线数据: {len(data)} 条记录")
            else:
                df = await self._fetch_monthly_by_date(monthly_date)
                if df.empty:
                    logger.warning(f"日期 {monthly_date} 的月线数据为空")
                    continue
                    
                data = df.to_dict('records')
                logger.info(f"获取到月线数据: {len(data)} 条记录")
    ...
    async def fetch_full_history(self, batch_size: int = 1, market_codes: List[str] = None):
        ...
        logger.info(f"获取完整历史月线数据: 从 {start_date} 到 {end_date}")
        ...
        logger.info(f"月线数据获取完成: 总共获取 {total_records} 条记录，成功批次: {success_batches}/{len(stock_batches)}")
        return True
    ...
    async def fetch_by_date_range(self, start_date: str, end_date: str, 
        ...
        按日期范围获取月线数据
        """
        logger.info(f"按日期范围获取月线数据: 从 {start_date} 到 {end_date}")
    ...
    def fetch_monthly_by_date(self, trade_date: str, ts_code: str = None) -> pd.DataFrame:
        """
        按日期获取月线行情数据
        ...
        try:
            ...
            logger.debug(f"获取日期 {trade_date} 的月线数据"+(f" 股票代码: {ts_code}" if ts_code else ""))
            ...
            logger.info(f"成功获取日期 {trade_date} 的月线数据"+(f" 股票代码: {ts_code}" if ts_code else f"，共 {len(df)} 条记录"))
            return df
        except Exception as e:
            logger.error(f"获取日期 {trade_date} 的月线数据失败: {str(e)}")
            return pd.DataFrame()
            
    def fetch_monthly_by_code(self, ts_code: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
        """
        按股票代码获取月线行情数据
        ...
        try:
            ...
            logger.debug(f"获取股票 {ts_code} 的月线数据"+(f" 日期范围: {start_date} 至 {end_date}" if start_date and end_date else ""))
            ...
            logger.info(f"成功获取股票 {ts_code} 的月线数据，共 {record_count} 条记录")
            return df
        except Exception as e:
            logger.error(f"获取股票 {ts_code} 的月线数据失败: {str(e)}")
            return pd.DataFrame()
    ...
    def fetch_data_parallel(self, ts_codes: Set[str], start_date: str, end_date: str, batch_size: int = 10) -> pd.DataFrame:
        ...
        logger.info(f"开始并行批量获取 {len(ts_codes_list)} 个股票的月线行情数据，分为 {total_batches} 个批次处理")
            ...
            try:
                ...
                try:
                    for code in batch_ts_codes:
                        try:
                            # 使用WAN接口获取数据
                            df = self.fetch_monthly_by_code_with_wan(code, start_date, end_date, wan_info)
                            if not df.empty:
                                # 立即保存数据
        ...
        logger.success(f"并行处理成功获取 {success_batches}/{total_batches} 个批次的月线行情数据，共 {total_records} 条记录，耗时 {elapsed_total:.2f}s")
    ...
    def fetch_data_batch(self, ts_codes: Set[str], start_date: str, end_date: str, batch_size: int = 100, minute_rate_limit: int = 500, hour_rate_limit: int = 4000) -> pd.DataFrame:
        ...
        # 配置参数
        total_batches = (len(ts_codes_list) + batch_size - 1) // batch_size
        logger.info(f"开始批量获取 {len(ts_codes_list)} 个股票的月线行情数据，分为 {total_batches} 个批次处理")
        ...
        # 处理完成
        elapsed_total = time.time() - start_time_total
        logger.success(f"批量处理成功获取 {success_batches}/{total_batches} 个批次的月线行情数据，共 {total_records} 条记录，耗时 {elapsed_total:.2f}s")
    ...
    def _fetch_batch(
        ...
        for ts_code in batch_ts_codes:
            df = self.fetch_monthly_by_code_with_offset(ts_code=ts_code, start_date=start_date, end_date=end_date)
            if not df.empty:
                all_data.append(df)
    ...
    def _fetch_sequential_by_dates(self, date_list: List[str], target_ts_codes: Set[str]) -> bool:
        ...
        # 逐日获取数据
        for date_str in date_list:
            logger.info(f"正在获取日期 {date_str} 的月线数据...")
            
            # 获取当日所有股票数据，使用支持offset的方法，不传递ts_code参数
            df = self.fetch_monthly_by_date_with_offset(trade_date=date_str)
            ...
            if not df.empty:
                ...
                if not df_filtered.empty:
                    ...
                    if success:
                        ...
                        logger.success(f"成功保存 {date_str} 的月线数据，{len(df_filtered)} 条记录，新增 {inserted}，更新 {updated}，跳过 {skipped}")
                    else:
                        logger.warning(f"保存 {date_str} 的月线数据失败")
                else:
        ...
        # 输出详细统计信息
        logger.success(f"按日期顺序获取月线数据完成，成功处理 {success_days}/{total_days} 天")
    ...
    def _fetch_parallel_by_dates(self, date_list: List[str], target_ts_codes: Set[str], available_wans: List[int]) -> bool:
            ...
            try:
                ...
                if not wan_info:
                    ...
                    try:
                        ...
                        df = self.fetch_monthly_by_date_with_wan(trade_date=date_str, wan_info=wan_info)
                    finally:
                        # 确保释放WAN端口
                        if wan_info:
                            # 添加短暂延迟，确保端口完全释放
        ...
        # 输出详细统计信息
        logger.success(f"按日期并行获取月线数据完成，成功处理 {success_days}/{total_days} 天")
        logger.info(f"数据统计: 总记录数 {total_records}，新增 {total_inserted}，更新 {total_updated}，跳过 {total_skipped}，耗时 {elapsed_total:.1f}s")
    ...
    def _run_by_stock_codes(self, start_date: str, end_date: str, batch_size: int = 10, use_parallel: bool = True) -> bool:
        ...
        logger.info(f"已获取 {len(target_ts_codes)} 个目标股票代码，将按ts_code分批获取日期范围 {start_date} 至 {end_date} 的月线数据")
    ...
    def run(self, config: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if full:
            ...
            end_date = datetime.now().strftime('%Y%m%d')
            logger.info(f"使用full模式：获取 {start_date} 至 {end_date} 期间的全部月线数据")
            ...
            logger.info(f"使用日期范围模式：获取 {start_date} 至 {end_date} 期间的月线数据")
            return self._run_by_stock_codes(start_date, end_date, batch_size, use_parallel)
    ...
    def fetch_monthly_by_date_with_wan(self, trade_date: str, ts_code: str = None, wan_info: Tuple[int, int] = None, max_count: int = 9000) -> pd.DataFrame:
        """
        使用WAN接口按日期获取月线行情数据，支持offset处理超过API限制的数据
        ...
        try:
            ...
            logger.debug(f"使用WAN接口获取日期 {trade_date} 的月线数据"+(f" 股票代码: {ts_code}" if ts_code else ""))
            ...
            if not wan_info:
                ...
                return self.fetch_monthly_by_date_with_offset(trade_date=trade_date, ts_code=ts_code, max_count=max_count)
            ...
            try:
                ...
                logger.info(f"WAN {wan_idx} 成功获取日期 {trade_date} 的月线数据"+(f" 股票代码: {ts_code}" if ts_code else f"，共 {total_records} 条记录"))
                return result_df
            finally:
                # 重置客户端设置，确保资源释放
            ...
            logger.error(f"WAN接口获取日期 {trade_date} 的月线数据失败: {str(e)}")
            import traceback
            logger.debug(f"详细错误信息: {traceback.format_exc()}")
            return pd.DataFrame()
            
    def fetch_monthly_by_code_with_wan(self, ts_code: str, start_date: str = None, end_date: str = None, wan_info: Tuple[int, int] = None, max_count: int = 9000) -> pd.DataFrame:
        """
        使用WAN接口按股票代码获取月线行情数据，支持offset处理超过API限制的数据
        ...
        try:
            ...
            logger.debug(f"使用WAN接口获取股票 {ts_code} 的月线数据"+(f" 日期范围: {start_date} 至 {end_date}" if start_date and end_date else ""))
            ...
            if not wan_info:
                logger.warning("未提供WAN接口信息，使用普通客户端获取数据")
                return self.fetch_monthly_by_code_with_offset(ts_code=ts_code, start_date=start_date, end_date=end_date, max_count=max_count)
            ...
            try:
                ...
                logger.info(f"WAN {wan_idx} 成功获取股票 {ts_code} 的月线数据，共 {total_records} 条记录")
                return result_df
            finally:
                # 重置客户端设置，确保资源释放
                if hasattr(client, 'reset_local_address'):
            ...
            logger.error(f"WAN接口获取股票 {ts_code} 的月线数据失败: {str(e)}")
            import traceback
            logger.debug(f"详细错误信息: {traceback.format_exc()}")
            return pd.DataFrame()

    def fetch_monthly_by_date_with_offset(self, trade_date: str, ts_code: str = None, max_count: int = 9000) -> pd.DataFrame:
        """
        按日期获取月线行情数据，支持offset处理超过API限制的数据
    ...
    def fetch_monthly_by_code_with_offset(self, ts_code: str, start_date: str = None, end_date: str = None, max_count: int = 9000) -> pd.DataFrame:
        """
        按股票代码获取月线行情数据，支持offset处理超过API限制的数据
        ...
        try:
            ...
            logger.info(f"成功获取股票 {ts_code} 的月线数据，共 {len(df)} 条记录")
            return df
        except Exception as e:
            logger.error(f"获取股票 {ts_code} 的月线数据失败: {str(e)}")
            return pd.DataFrame()
    ...
    def _get_last_trading_days_of_weeks(self, date_list: List[str]) -> List[str]:
        ...
        for date_str in sorted_dates:
            ...
            if current_week is None:
                ...
                monthly_last_trading_days.append(last_date)
                current_week = year_week
                last_date = date_str
            else:
                # 同一周内，更新last_date
...
def main():
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="月线行情数据获取工具")
    ...
    # 获取月线行情数据
    if args.mock:
        logger.warning("使用模拟数据模式，生成随机的月线行情数据")
        # 使用mock数据的逻辑可以在这里实现
    else:
        if args.ts_code:
            ...
            logger.info(f"获取单个股票 {args.ts_code} 的月线数据，日期范围: {start_date} 至 {end_date}")
            
            # 使用支持offset的方法获取数据
            result_df = fetcher.fetch_monthly_by_code_with_offset(
            ...
            if result:
                logger.success("月线数据获取和保存成功")
            else:
                logger.error("月线数据获取或保存失败")
