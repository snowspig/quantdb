#!/usr/bin/env python
"""
Monthly Basic Fetcher V2 - 获取日线基本数据并保存到MongoDB
...
使用方法：
    python monthly_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python monthly_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python monthly_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python monthly_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python monthly_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python monthly_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
# 添加月末日期检测函数
def is_month_end_date(date_str: str) -> bool:
    """
    判断日期是否为月末日期
    ...
    try:
        ...
        # 如果下一天是下个月的第一天，那么当前日期就是月末
        return next_day.month != date.month
    except Exception as e:
        logger.error(f"判断月末日期时出错: {str(e)}")
        return False

class monthlyFetcher(TushareFetcher):
    """
    日线基本数据获取器V2
    ...
    def get_trade_dates(self, start_date: str = None, end_date: str = None) -> List[str]:
        ...
        try:
            ...
            # 去重并排序
            all_trade_dates = sorted(list(set(trade_dates_with_duplicates)))
            
            if all_trade_dates:
                logger.info(f"从trade_cal集合获取到 {len(all_trade_dates)} 个不重复的交易日 (所有交易所)")
            else:
                logger.warning("trade_cal集合中未找到符合条件的交易日数据")
                raise Exception("未找到交易日数据")
        except Exception as e:
            ...
            while current_date <= end_date_obj:
                all_trade_dates.append(current_date.strftime('%Y%m%d'))
                current_date += timedelta(days=1)
            
            logger.info(f"生成日期范围内的所有日期，共 {len(all_trade_dates)} 个日期")
        
        # 筛选月末交易日
        month_end_dates = [date for date in all_trade_dates if is_month_end_date(date)]
        
        if month_end_dates:
            logger.info(f"筛选出 {len(month_end_dates)}/{len(all_trade_dates)} 个月末交易日")
        else:
            logger.warning(f"日期范围 {start_date} 到 {end_date} 内没有月末交易日")
        
        return month_end_dates
    
    def fetch_monthly_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的数据
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_monthly_data(trade_date)
                if df is None or df.empty:
                    return False
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if self.full_mode:
                ...
                # 第二步：获取日期范围内的月末交易日
                logger.info(f"第二步：获取日期范围 {self.start_date} - {self.end_date} 内的月末交易日...")
                trade_dates = self.get_trade_dates(self.start_date, self.end_date)
                
                if not trade_dates:
                    logger.warning("未找到月末交易日，没有数据需要处理")
                    # 没有月末交易日时直接返回成功，不进行数据处理
                    return True  
                
                # 第三步：获取数据（串行或并行）
                if self.serial_mode:
                    # 串行模式
                    logger.info(f"第三步：串行处理 {len(trade_dates)} 个月末交易日的数据")
                    all_success = True
                    ...
                    for trade_date in trade_dates:
                        ...
                        logger.info(f"正在处理月末交易日: {trade_date}")
                    ...
                    # 并行模式
                    logger.info(f"第三步：并行处理 {len(trade_dates)} 个月末交易日的数据")
...
def main():
    ...
    parser.add_argument('--collection-name', default='monthly', help='MongoDB集合名称')
