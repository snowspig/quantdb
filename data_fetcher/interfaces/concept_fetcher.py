#!/usr/bin/env python
"""
concept Basic Fetcher V2 - 获取概念股分类并保存到MongoDB

该脚本用于从湘财Tushare获取概念股分类，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=10110

使用方法：
    python concept_fetcher.py                   # 使用湘财真实API数据，简洁日志模式
    python concept_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python concept_fetcher.py --exchange SZSE   # 获取深交所的股票数据
    python concept_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）

注意：concept接口使用limit和offset参数分批获取所有数据，直到数据量小于limit停止
"""
import sys
import time
import json
...
class conceptFetcher(TushareFetcher):
    """
    概念股分类获取器V2
    
    该类用于从Tushare获取概念股分类并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "concept.json",
        db_name: str = None,
        collection_name: str = "concept",
        ...
        logger.info("抓取模式: 按股票代码逐个抓取(concept接口限制)")
    ...
    def fetch_concept_data(self, ann_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定公告日期的数据
        ...
        if not stock_codes:
            logger.error("未能获取到股票代码列表，无法获取概念股分类数据")
            return None
    ...
    def _process_date_with_wan(self, ann_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_concept_data(ann_date)
                if df is None or df.empty:
                    return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取概念股分类并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='concept', help='MongoDB集合名称')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("概念股分类获取和保存成功")
                return 0
            else:
                logger.error("概念股分类获取或保存失败")
                return 1
        except KeyboardInterrupt:
