#!/usr/bin/env python
"""
fin_mainbz Fetcher - 获取主营业务构成并保存到MongoDB

该脚本用于从湘财Tushare获取主营业务构成，并保存到MongoDB数据库中
...
使用方法：
    python fin_mainbz_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python fin_mainbz_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python fin_mainbz_fetcher.py --end-date 20200630  # 指定报告期
    python fin_mainbz_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python fin_mainbz_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class fin_mainbzFetcher(TushareFetcher):
    """
    日线基本数据获取器V2
    ...
    def __init__(
        ...
        interface_name: str = "fin_mainbz.json",
        db_name: str = None,
        collection_name: str = "fin_mainbz",
        start_date: str = None,
        end_date: str = None,
    ...
    def fetch_fin_mainbz_data(self, end_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定报告期的管理层薪酬及持股数据
    ...
    def _process_date_with_wan(self, end_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_fin_mainbz_data(end_date)
                if df is None or df.empty:
                    return False
    ...
    def get_quarter_end_dates(self, start_date=None, end_date=None, recent=False) -> List[str]:
        ...
        管理层薪酬及持股数据(fin_mainbz)只在半年报和年报时发布，
...
def main():
    ...
    parser.add_argument('--collection-name', default='fin_mainbz', help='MongoDB集合名称')
