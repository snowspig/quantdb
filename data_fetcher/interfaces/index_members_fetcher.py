#!/usr/bin/env python
"""
指数成分股数据获取器 - 获取A股指数成分股数据并保存到MongoDB

该脚本用于从湘财Tushare获取A股指数成分股数据，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与daily_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=96

使用方法：
    python index_members_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python index_members_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python index_members_fetcher.py --ts-code 000300.SH  # 指定获取沪深300指数成分股
    python index_members_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python index_members_fetcher.py --full            # 使用完整模式，获取所有指数的成分股数据
"""
import sys
import time
import json
...
class IndexMembersFetcher(TushareFetcher):
    """
    指数成分股数据获取器
    
    该类用于从Tushare获取指数成分股数据并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "index_members.json",
        db_name: str = None,
        collection_name: str = "index_members",
        ts_code: str = None,  # 指定获取的指数代码
        start_date: str = None,
        end_date: str = None,
        verbose: bool = False,
        ...
        full_mode: bool = False,  # 是否使用完整模式（获取所有指数成分股）
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            result = self.client.get_data(
                api_name='index_members', 
                params=params,
                wan_idx=wan_idx # 传递 wan_idx
            )
            
            # 字段重命名：ts_code -> index_code, con_ts_code -> ts_code
            if result is not None and not result.empty:
                if 'ts_code' in result.columns:
                    result.rename(columns={'ts_code': 'index_code'}, inplace=True)
                if 'con_ts_code' in result.columns:
                    result.rename(columns={'con_ts_code': 'ts_code'}, inplace=True)
    ...
    def fetch_index_members(self, ts_code: str) -> Optional[pd.DataFrame]:
        """获取指定指数代码的成分股数据"""
        logger.info(f"正在获取指数 {ts_code} 的成分股数据...")
    ...
    def get_index_list(self) -> List[str]:
        ...
        从index_basic集合获取指数列表，如果失败则使用预定义的主要指数列表
        ...
        try:
            ...
            # 查询index_basic集合
            result = self.mongodb_handler.find_documents("index_basic", {})
            
            # 提取指数代码列表
            index_codes = [doc.get("index_code") for doc in result if "index_code" in doc]
            
            if index_codes:
                # 去重并排序
                index_codes = sorted(list(set(index_codes)))
                logger.info(f"从index_basic集合获取到 {len(index_codes)} 个不重复的指数代码")
                return index_codes
            else:
                logger.warning("index_basic集合中未找到符合条件的指数代码")
        except Exception as e:
            logger.error(f"查询index_basic指数列表失败: {str(e)}")
        
        # 使用预定义的主要指数列表作为备选
        logger.warning("无法从index_basic获取指数列表，将使用预定义的主要指数列表")
        ...
        logger.info(f"使用预定义的主要指数列表，共 {len(main_indices)} 个指数")
        return main_indices
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # 根据模式走不同的处理流程
            if self.full_mode:
                # 完整模式：按分页方式获取所有指数成分股数据
                logger.info("使用完整模式，按分页方式获取所有指数成分股数据")
                ...
                # 如果指定了特定指数，则只处理该指数
                index_list = [self.ts_code]
                logger.info(f"将只处理指定的指数: {self.ts_code}")
            else:
                # 默认情况：使用主要指数列表
                index_list = self.get_index_list()
                logger.info(f"将处理主要指数，共 {len(index_list)} 个")
            
            if not index_list:
                logger.warning("未找到需要处理的指数，没有数据需要获取")
                return True  # 没有数据也视为成功
            
            # 第三步：获取数据
            logger.info(f"第三步：处理 {len(index_list)} 个指数的成分股数据")
            all_success = True
            ...
            for index_code in index_list:
                ...
                logger.info(f"正在处理指数: {index_code}")
                
                try:
                    # 获取指数成分股数据
                    df = self.fetch_index_members(index_code)
                    if df is None or df.empty:
                        logger.warning(f"指数 {index_code} 的成分股数据为空或获取失败")
                        continue
                    
                    # 记录获取到的数据量
                    record_count = len(df)
                    logger.info(f"获取到指数 {index_code} 的 {record_count} 条成分股记录")
                    
                    # 保存数据到MongoDB
                    success = self.save_to_mongodb(df)
                    if success:
                        logger.success(f"指数 {index_code} 的成分股数据已保存到MongoDB")
                    else:
                        logger.error(f"保存指数 {index_code} 的成分股数据到MongoDB失败")
                        all_success = False
                    
                    # 增加随机延时，避免API限制
                    if index_code != index_list[-1]:  # 不是最后一个指数
                        delay = random.uniform(1, 2)
                        logger.debug(f"添加 {delay:.2f} 秒延时，避免API限制")
                        time.sleep(delay)
                        
                except Exception as e:
                    logger.error(f"处理指数 {index_code} 的成分股数据时发生异常: {str(e)}")
                    all_success = False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取指数成分股数据并保存到MongoDB')
    parser.add_argument('--config', default='config/config.yaml', help='配置文件路径')
    parser.add_argument('--interface-dir', default='config/interfaces', help='接口配置文件目录')
    parser.add_argument('--ts-code', help='指定要获取的指数代码，如000300.SH')
    ...
    parser.add_argument('--collection-name', default='index_members', help='MongoDB集合名称')
    ...
    parser.add_argument('--full', action='store_true', help='使用完整模式，获取所有指数的成分股数据')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("指数成分股数据获取和保存成功")
                return 0
            else:
                logger.error("指数成分股数据获取或保存失败")
                return 1
