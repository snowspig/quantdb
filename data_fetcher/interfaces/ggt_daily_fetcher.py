#!/usr/bin/env python
"""
ggt_daily Basic Fetcher V2 - 获取港股通每日成交统计数据并保存到MongoDB

该脚本用于从湘财Tushare获取港股通每日成交统计数据，并保存到MongoDB数据库中
该版本继承TushareFetcher基类，实现了与stock_basic_fetcher相同的架构和功能

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=26

使用方法：
    python ggt_daily_fetcher.py                   # 使用湘财真实API数据，简洁日志模式，获取近期数据
    python ggt_daily_fetcher.py --verbose         # 使用湘财真实API数据，详细日志模式
    python ggt_daily_fetcher.py --start-date 20200101 --end-date 20231231  # 指定日期范围
    python ggt_daily_fetcher.py --exchange SZSE   # 获取深交所的日线数据
    python ggt_daily_fetcher.py --serial          # 使用串行模式处理数据（默认为并行模式）
    python ggt_daily_fetcher.py --full            # 使用完整模式，按股票代码列表获取所有股票的基本数据
"""
import sys
import time
import json
...
class ggt_dailyFetcher(TushareFetcher):
    """
    港股通每日成交统计数据获取器V2
    
    该类用于从Tushare获取港股通每日成交统计数据并保存到MongoDB数据库
    ...
    def __init__(
        ...
        interface_name: str = "ggt_daily.json",
        db_name: str = None,
        collection_name: str = "ggt_daily",
        start_date: str = None,
        end_date: str = None,
        ...
        # 保存full_mode状态，在full模式下修改日期范围
        if self.full_mode:
            # 在完整模式下，设置日期范围为全部历史数据
            today = datetime.now().strftime("%Y%m%d")
            self.start_date = '19900101'  # 设置为1990年1月1日起
            self.end_date = today
            logger.info(f"完整模式下设置日期范围为: {self.start_date} - {self.end_date}")
        else:
            # 设置默认日期范围（如果未提供）
    ...
    def fetch_data(self, **kwargs) -> Optional[pd.DataFrame]:
        ...
        # 参数检查：至少需要 trade_date 或 start_date+end_date
        if not (trade_date or (start_date and end_date)):
            logger.error("必须提供 trade_date 或 start_date+end_date")
            return None
    ...
    def process_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        由于ggt_daily接口返回的是市场数据，不需要按股票代码前缀过滤
        ...
        # 直接返回数据，不需要任何过滤
        logger.debug("ggt_daily接口数据不需要按股票代码过滤")
        return df
    ...
    def fetch_market_data(self, market_type: str) -> Optional[pd.DataFrame]:
        ...
        # 在full模式下设置最大日期范围（从19900101到当前日期）
        if self.full_mode:
            today = datetime.now().strftime("%Y%m%d")
            params['start_date'] = '19900101'
            params['end_date'] = today
            logger.info(f"完整模式下使用日期范围: 19900101 - {today}")
        else:
            # 非full模式使用用户指定的日期范围
    ...
    def _process_market_parallel(self, market_types: List[str]) -> bool:
        ...
        def process_wan_markets(wan_idx, markets):
            ...
            try:
                ...
                # 在full模式下设置最大日期范围（从19900101到当前日期）
                start_date = '19900101' if self.full_mode else self.start_date
                end_date = datetime.now().strftime("%Y%m%d") if self.full_mode else self.end_date
                ...
                for market_type in markets:
                    ...
                    try:
                        ...
                        # 获取单个市场数据，直接传递wan_idx参数，同时传递日期参数
                        df = self.fetch_data(
    ...
    def fetch_ggt_daily_data(self, trade_date: str) -> Optional[pd.DataFrame]:
        """
        获取指定交易日的数据
    ...
    def _process_date_with_wan(self, trade_date: str, wan_idx: int) -> bool:
        ...
        if wan_idx not in self.wan_locks:
            ...
            # 尝试使用默认处理方式
            try:
                df = self.fetch_ggt_daily_data(trade_date)
                if df is None or df.empty:
                    return False
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if self.full_mode:
                # 完整模式：按市场类型获取
                logger.info(f"使用完整模式，按市场类型获取所有历史数据（从{self.start_date}到{self.end_date}）")
                ...
                # 第三步：获取数据（串行或并行）
                logger.info(f"第三步：处理 {len(market_types)} 个市场类型的数据，日期范围: {self.start_date} - {self.end_date}")
    ...
    def _process_data_full(self) -> bool:
        """
        全量下载模式的处理逻辑
        1. 获取所有交易日
        2. 对每个交易日按市场类型获取数据
        ...
        """
        # 获取所有交易日
        cal_list = self._get_trade_cal()
        if cal_list is None or len(cal_list) == 0:
            logger.error("无法获取交易日历")
            return False

        # 使用四种市场类型
        market_types = [1, 2, 3, 4]  # 1：沪股通 2：港股通（沪） 3：深股通 4：港股通（深）
        ...
        try:
            ...
            # 等待所有任务完成
            self._wait_for_tasks(futures)
            
            return True
            
        except Exception as e:
            logger.error(f"全量获取数据时出错: {str(e)}")
            return False
        finally:
            self._shutdown_executor()
            
    def fetch_market_data(self, market_type, trade_date):
        """
        获取特定市场类型和交易日的数据
        
        Args:
            market_type: 市场类型 (1-4)
            trade_date: 交易日期
        """
        try:
            # 使用随机WAN口获取数据
            wan_idx = random.randint(0, self.config.wan_count - 1) if self.config.wan_count > 0 else None
            ...
            # 处理数据
            if df is not None and not df.empty:
                self.process_data(df, trade_date)
                logger.info(f"成功获取并存储市场类型 {market_type} 在 {trade_date} 的数据")
            else:
                logger.warning(f"市场类型 {market_type} 在 {trade_date} 没有数据")
                
        except Exception as e:
            logger.error(f"获取市场类型 {market_type} 在 {trade_date} 的数据时出错: {str(e)}")
            # 如果需要，可以在这里添加重试逻辑
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取港股通每日成交统计数据并保存到MongoDB')
    ...
    parser.add_argument('--collection-name', default='ggt_daily', help='MongoDB集合名称')
    ...
    try:
        ...
        try:
            ...
            if success:
                logger.success("港股通每日成交统计数据获取和保存成功")
                return 0
            else:
                logger.error("港股通每日成交统计数据获取或保存失败")
                return 1
