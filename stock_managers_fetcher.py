...
class StockManagersFetcher:
    ...
    def fetch_stk_managers_by_date(self, start_date: str, end_date: str, ts_code: str = None, wan_idx: int = None, port: int = None) -> Optional[pd.DataFrame]:
        """
        按公告日期范围获取上市公司管理层信息
        ...
        for retry in range(self.retry_count + 1):
            try:
                ...
                # 调用Tushare API
                msg = f"正在从湘财Tushare获取 {start_date} 至 {end_date} 期间的上市公司管理层信息"
                if retry > 0:
                    msg += f" (重试 {retry}/{self.retry_count})"
                logger.info(msg)
                ...
                logger.success(f"成功获取 {start_date} 至 {end_date} 期间 {len(df)} 条上市公司管理层信息，耗时 {elapsed:.2f}s")
                ...
                logger.error(f"获取 {start_date} 至 {end_date} 期间上市公司管理层信息失败: {str(e)}")
                import traceback
                logger.debug(f"详细错误信息: {traceback.format_exc()}")
    ...
    def _fetch_data_for_date_range(self, date_range: Tuple[str, str], wan_info=None, ts_code=None) -> Optional[pd.DataFrame]:
        ...
        if wan_info:
            wan_idx, port = wan_info
            return self.fetch_stk_managers_by_date(start_date, end_date, ts_code, wan_idx, port)
        else:
            return self.fetch_stk_managers_by_date(start_date, end_date, ts_code)
...
def main():
    ...
    # 创建获取器
    fetcher = StockManagersFetcher(verbose=args.verbose)
