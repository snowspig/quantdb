#!/usr/bin/env python
"""
上市公司管理层信息获取器 - 获取上市公司管理层信息并保存到MongoDB

该脚本用于从湘财Tushare获取上市公司管理层信息，并保存到MongoDB数据库中，仅保留00、30、60、68板块的股票
该版本通过分时间段获取和多WAN接口并行处理，解决大量数据获取问题

参考接口文档：http://tushare.xcsc.com:7173/document/2?doc_id=10193

使用方法：
    python stock_managers_fetcher.py              # 默认使用recent模式获取最近一周的数据更新
    python stock_managers_fetcher.py --full        # 获取完整历史数据而非默认的最近一周数据
    python stock_managers_fetcher.py --verbose     # 使用湘财真实API数据，详细日志模式
    python stock_managers_fetcher.py --mock        # 使用模拟数据模式（API不可用时）
    python stock_managers_fetcher.py --start-date 20100101 --end-date 20201231  # 指定日期范围获取数据
    python stock_managers_fetcher.py --recent      # 显式指定recent模式（最近一周数据更新，默认模式）
"""
import os
import sys
import json
...
class StockManagersFetcher:
    """
    上市公司管理层信息获取器
    
    该类用于从Tushare获取上市公司管理层信息并保存到MongoDB数据库，支持按市场代码过滤
    优化点：
    ...
    def fetch_stk_managers_by_code(self, ts_code: str, wan_idx: int = None, port: int = None) -> Optional[pd.DataFrame]:
        """
        按股票代码获取上市公司管理层信息
        ...
        Returns:
            上市公司管理层信息DataFrame，如果失败则返回None
        """
        # 添加重试机制
        for retry in range(self.retry_count + 1):
            try:
                # 准备参数
                api_name = self.interface_config.get("api_name", "stk_managers")
                ...
                # 调用Tushare API
                msg = f"正在从湘财Tushare获取股票 {ts_code} 的上市公司管理层信息"
                if retry > 0:
                ...
                logger.success(f"成功获取股票 {ts_code} 的 {len(df)} 条上市公司管理层信息，耗时 {elapsed:.2f}s")
                ...
                logger.error(f"获取股票 {ts_code} 的上市公司管理层信息失败: {str(e)}")
                import traceback
    ...
    def _fetch_data_for_code(self, ts_code: str, wan_info=None) -> Optional[pd.DataFrame]:
        ...
        if wan_info:
            wan_idx, port = wan_info
            return self.fetch_stk_managers_by_code(ts_code, wan_idx, port)
        else:
            return self.fetch_stk_managers_by_code(ts_code)
    ...
    def fetch_stk_managers_by_date(self, start_date: str, end_date: str, ts_code: str = None, wan_idx: int = None, port: int = None, batch_size: int = 10000) -> Optional[pd.DataFrame]:
        """
        按公告日期范围获取上市公司管理层信息，支持分批获取以避免超过API限制
        ...
        while has_more:
            ...
            for retry in range(self.retry_count + 1):
                try:
                    # 调用Tushare API
                    msg = f"正在从湘财Tushare获取 {start_date} 至 {end_date} 期间的上市公司管理层信息 (页码: {page+1}, 偏移: {page * batch_size})"
                    if retry > 0:
                        msg += f" (重试 {retry}/{self.retry_count})"
                    logger.info(msg)
        ...
        logger.success(f"成功获取 {start_date} 至 {end_date} 期间总计 {len(combined_df)} 条上市公司管理层信息")
    ...
    def _fetch_data_for_date_range(self, date_range: Tuple[str, str], wan_info=None, ts_code=None) -> Optional[pd.DataFrame]:
        ...
        if wan_info:
            wan_idx, port = wan_info
            return self.fetch_stk_managers_by_date(start_date, end_date, ts_code, wan_idx, port, self.batch_size)
        else:
            return self.fetch_stk_managers_by_date(start_date, end_date, ts_code, batch_size=self.batch_size)
                
    def fetch_stk_managers(self, ts_codes: Set[str] = None, start_date=None, end_date=None) -> Optional[pd.DataFrame]:
        """
        获取上市公司管理层信息
...
def main():
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="上市公司管理层数据获取工具")
    ...
    # 创建获取器
    fetcher = StockManagersFetcher(verbose=args.verbose, batch_size=args.batch_size)
    
    # 获取上市公司管理层数据
    if args.mock:
        logger.warning("使用模拟数据模式，生成随机的上市公司管理层信息")
        # 使用mock数据的逻辑（暂未实现）
    else:
        # 设置日期范围
        ...
        if start_date and end_date:
            ...
            # 打印日期范围信息
            logger.info(f"开始获取上市公司管理层数据，日期范围: {start_date or '1990年起'} 至 {end_date or '当前'}")
            ...
            # 并行获取各时间段的数据
            start_time = time.time()
            logger.info(f"开始分段获取上市公司管理层数据，共 {len(date_ranges)} 个时间段")
            ...
            if not df.empty:
                # 确定去重的列，针对管理层数据，可能的主键是ts_code, name和begin_date的组合
                if all(col in df.columns for col in ["ts_code", "name", "begin_date"]):
                    df = df.drop_duplicates(subset=["ts_code", "name", "begin_date"])
            ...
            # 获取上市公司管理层数据
            df = fetcher.fetch_stk_managers(ts_codes)
        ...
        if df is not None and not df.empty:
            ...
            if not df_filtered.empty:
                ...
                logger.error("没有符合条件的上市公司管理层数据可保存")
        else:
            logger.error("未获取到上市公司管理层数据")
